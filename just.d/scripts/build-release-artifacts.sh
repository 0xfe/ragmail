...
usage() {
  cat <<'EOF'
usage: build-release-artifacts.sh [--output-dir dist/release] [--version X.Y.Z] [--host-triple triple]
...
binary_path="rust/target/release/ragmail-rs"
if [[ ! -x "${binary_path}" ]]; then
  echo "release binary missing at ${binary_path}" >&2
  exit 1
fi
...
if [[ "${host_triple}" == *"unknown-linux-gnu" ]] && command -v dpkg-deb >/dev/null 2>&1; then
  deb_path="${output_dir}/ragmail-rs_${version}_${deb_arch}.deb"
