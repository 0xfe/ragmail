...
class TestEmbeddingBenchmark:
    ...
    def require_opt_in(self):
        """Skip heavyweight model benchmarks unless explicitly requested."""
        if os.environ.get("RAGMAIL_RUN_EMBEDDING_BENCHMARKS") != "1":
            pytest.skip(
                "Set RAGMAIL_RUN_EMBEDDING_BENCHMARKS=1 to run embedding benchmark tests"
            )
...
class TestEmbeddingProviderComparison:
    ...
    @pytest.fixture(autouse=True)
    def require_opt_in(self):
        """Skip heavyweight provider benchmarks unless explicitly requested."""
        if os.environ.get("RAGMAIL_RUN_EMBEDDING_BENCHMARKS") != "1":
            pytest.skip(
                "Set RAGMAIL_RUN_EMBEDDING_BENCHMARKS=1 to run embedding benchmark tests"
            )
